...
function renderRoster() {
  ...
  $('myRoster').innerHTML = roster.length ? roster.map(p => {
    const rankedPlayer = ranked.find(x => x.name?.toLowerCase() === String(p.name || '').toLowerCase());
    const bye = p.bye ?? rankedPlayer?.bye ?? '';
    return `<div class="roster-player"><span class="roster-pos">${esc(p.position)}</span><div><strong>${esc(p.name)}</strong><br><small>R${p.round} · #${p.overall} · ${esc(p.nflTeam || '')}${bye ? ` · Bye ${esc(bye)}` : ''}</small></div></div>`;
