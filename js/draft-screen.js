...
watchState(s => { state = s || {}; $('season').textContent = Number(state.season || board.season || DRAFT.season); renderTimer(); });
watchBoard(b => {
...
async function showPickSequence(p) {
  ...
  const speechSeason = Number(state.season || board.season || p.season || DRAFT.season);
